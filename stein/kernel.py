...
def hamming_kernel(x: List[int], xp: List[int]):
    assert len(x) == len(xp)
    diff = [1 if x[i] != xp[i] else 0 for i in range(len(x))]
    return np.exp(-np.sum(diff) / len(x))


def laplace_kernel(x: List[int] or List[float], xp: List[int] or List[float]):
    assert len(x) == len(xp)
    dist = np.abs(x - xp)
    return np.exp(-np.sum(dist))


def gaussian_kernel(x: List[int] or List[float], xp: List[int] or List[float]):
    assert len(x) == len(xp)
    dist = LA.norm(x - xp)
...
class SteinBasis:
    ...
    def get_basis(
        self, X: List[int] or List[List[int]], vartype: Vartype
    ) -> List[List[float]]:
